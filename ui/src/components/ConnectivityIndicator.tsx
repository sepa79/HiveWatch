...
export function ConnectivityIndicator() {
  ...
  const title = useMemo(() => {
    ...
    if (result.state === 'ok') return 'Connectivity: OK (click for diagnostics)'
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const ok = await fetchHealth('/actuator/health', controller.signal)
        if (stopped) return
        if (ok) {
  ...
  return (
    <button type="button" className="iconButton iconButtonBare" title={title} onClick={() => navigate('/diagnostics')}>
